...
def get_answer(col, question: str, history: list, api_key: str, model: str) -> tuple[str, list[str], dict]:
    ...
    system = (
        ...
        "## HOTEL CATEGORY SELECTION (MANDATORY)\n"
        "Hotel/accommodation selection MUST be driven by:\n"
        "  (1) User's requested hotel category, AND\n"
        "  (2) Hotel data available in the knowledge base for each destination.\n\n"
        "STEP A — If the user has NOT provided a hotel category, ask:\n"
        "  'Which hotel category would you prefer? Available options are:\n"
        "   5 Star | 4 Star – D | 4 Star – C | 4 Star – B | 4 Star – A | 3 Star\n"
        "   (or any other category listed in the KB for this destination)'\n"
        "  Do NOT assume or default to any category.\n\n"
        "STEP B — Category normalization:\n"
        "  '5 Star' = '5*' = '5-Star' = '5 star hotel'\n"
        "  '4 Star – A' = '4* A' = '4-Star A' = '4 Star A'\n"
        "  '3 Star' = '3*' = '3-Star'\n"
        "  Normalize user input before matching against the KB.\n\n"
        "STEP C — For EACH destination in the itinerary:\n"
        "  1. Search the KB for: <Destination> → <Requested Category> → hotel names\n"
        "  2. If the category IS available: select one hotel (or list options for user to choose)\n"
        "  3. If the category IS NOT available: tell the user:\n"
        "     'The requested category is not available for [Destination]. "
        "Available categories: [list]. Which would you prefer?'\n"
        "  4. NEVER downgrade or upgrade category silently.\n\n"
        "STEP D — Hotel name rules:\n"
        "  - Use the EXACT hotel name from the KB. NEVER invent hotel names.\n"
        "  - If the KB entry says 'Royal Tulip / Artizan Design / Similar', pick one or list them.\n"
        "  - Preserve 'Similar' where it appears in the KB — do not replace it with a made-up name.\n\n"
